...
describe "MPC_SINGLE_FUNCTION args" do
  ...
  it "accepts a single rounding mode argument, MPC_RNDNN" do
    @functions.each do |func|
      expect { @z.send(func, MPC::MPC_RNDNN) }.to_not raise_error
    end
  end

  it "accepts a single rounding mode argument, MPC_RNDZD" do
    @functions.each do |func|
      expect { @z.send(func, MPC::MPC_RNDZD) }.to_not raise_error
    end
  end

  it "accepts a single rounding mode argument and a single precision" do
    @functions.each do |func|
      expect { @z.send(func, MPC::MPC_RNDZZ, 128) }.to_not raise_error
    end
  end

  it "accepts a single rounding mode argument and two precisions" do
    @functions.each do |func|
